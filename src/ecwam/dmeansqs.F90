...
!     INITIALISE
      MSSXX(KIJS:KIJL) = 0._JWRB
      MSSYY(KIJS:KIJL) = 0._JWRB
      MSSXY(KIJS:KIJL) = 0._JWRB

!     CALCULATE INTEGRATION UPPER LIMIT (IN UNITS OF INDEX)
      IF (XKMSS .EQ. 0._JWRB) THEN
...
!     CROSS TERM
      WANG(:) = COS(TH(:))*SIN(TH(:))
      CALL MEANSQS_LF(NFRE_EFF, KIJS, KIJL, F, WAVNUM, MSSXY(KIJS:KIJL), WANG)

...
!         11. WHITECAP FRACTION
        ...
        IF (LWMSS) THEN
!         12. DIRECTIONAL MEAN SQUARE SLOPE ALONG X
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' mssxx'
          DEFVAL(IFLDOFFSET)=0.0_JWRB  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY
                                       ! THE WAVE MODEL ICE FREE SEA POINTS.
!         13. DIRECTIONAL MEAN SQUARE SLOPE ALONG Y
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' mssyy'
          DEFVAL(IFLDOFFSET)=0.0_JWRB  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY
                                       ! THE WAVE MODEL ICE FREE SEA POINTS.
!         14. DIRECTIONAL MEAN SQUARE SLOPE CROSS-COMPONENT
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' mssxy'
          DEFVAL(IFLDOFFSET)=0.0_JWRB  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY
                                       ! THE WAVE MODEL ICE FREE SEA POINTS.
        ENDIF
...
!         IT MIGHT ALSO BE USED FOR NEMO !!!!!!!!!!
          ...
          IF (LWMSS) THEN
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%MSSXX(KIJS:KIJL,ICHNK)

            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%MSSYY(KIJS:KIJL,ICHNK)

            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%MSSXY(KIJS:KIJL,ICHNK)
          ENDIF
